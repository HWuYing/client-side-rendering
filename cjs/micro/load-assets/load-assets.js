...
let LoadAssets = class LoadAssets {
    ...
    readJavascript({ javascript, script, ...other }) {
        return (0, import_rxjs_1.forkJoin)(javascript.map((src) => this.http.getText(src))).pipe((0, import_rxjs_1.map)((js) => ({ script: js, javascript, ...other })));
    }
    createMicroTag(microName, staticAssets) {
        ...
        (0, import_rxjs_1.tap)((linkToStyles) => new Function((0, micro_1.createMicroElementTemplate)(microName, { linkToStyles }))()), (0, import_rxjs_1.map)(() => staticAssets));
    }
    readMicroStatic(microName) {
        const { assetsPath } = this.options;
        return this.http.get(assetsPath(microName)).pipe((0, import_rxjs_1.switchMap)((result) => this.parseStatic(microName, result)), (0, import_rxjs_1.switchMap)((result) => this.createMicroTag(microName, result)));
    }
};
